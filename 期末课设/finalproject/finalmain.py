...
# 加载数据
data = scipy.io.loadmat('横着走然后向后.mat')
...
def plot_tracking(cf_tracks, sumpoint, frame_num, colors, ax):
    ax.cla()
    ax.set_xlim(-3, 3)
    ax.set_ylim(0, 3)
    ax.set_xlabel('X 坐标')
    ...
    # 绘制当前检测点
    if sumpoint.size > 0:
        ax.scatter(sumpoint[:, 0], sumpoint[:, 1], s=100, marker='s', color=[0.5, 0.5, 0.5], label='检测点')
    ...
    for i in range(frames-42):
        ...
        if remaining_detections.size > 0:
            for det in remaining_detections:
                ...
                print(f'创建新轨迹，ID: {new_track.id}, 帧: {i + 1}, 位置: ({det[0]:.2f}, {det[1]:.2f})')

        # 删除未确认的老轨迹已经在 update_tracks 中处理

        # 绘制跟踪结果
        plot_tracking(cf_tracks, remaining_detections, i + 1, colors, ax)
        plt.pause(0.01)
